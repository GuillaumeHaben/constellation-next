...
export function ModalClub({ isOpen, onOpenChange, onCreate }) {
    // Controlled mode: parent provides `isOpen` and `onOpenChange`.
    ...
    const handleSubmit = async (e, internalOnClose) => {
        ...
        if (!form.checkValidity()) {
            // this triggers native browser validation UI
            form.reportValidity();
            return;
        }
        ...
        <Modal isOpen={isOpen} placement="top-center" onOpenChange={onOpenChange}>
            <ModalContent>
                {(internalOnClose) => (
                    <>
                        <Form>
                            <ModalHeader className="flex flex-col gap-1">Club view</ModalHeader>
                            <ModalBody>
                                <Input
                                    label="Name"
                                    isRequired
